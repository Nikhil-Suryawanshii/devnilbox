...
class SellerChatController extends Controller
{
    ...
    public function blockCustomer(Request $request)
    {
        ...
        ShopUserRepository::query()->updateOrCreate(
            ['shop_id' => $shop->id, 'user_id' => (int) $request->user_id],
            ['seller_blocked_at' => now()]
        );

        return $this->json('customer blocked successfully', []);
    }
    ...
    public function unblockCustomer(Request $request)
    {
        ...
        $shopUser->update(['seller_blocked_at' => null]);

        return $this->json('customer unblocked successfully', []);
    }
}
