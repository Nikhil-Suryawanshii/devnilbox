...
use App\Http\Requests\IndividualSellerLoginRequest;
...
class LoginController extends Controller
{
    ...
    /**
     * Individual seller registration with shop creation.
     * User can register with either email or phone number.
     *
     * @param  IndividualSellerLoginRequest  $request
     * @return \Illuminate\Http\Response
     */
    public function individualSellerLogin(IndividualSellerLoginRequest $request)
    {
        // Determine contact method (email or phone)
        $contact = $request->email ?? $request->phone;
        
        // Check if user already exists by email or phone
        $existingUser = UserRepository::findByContact($contact);
        if (!$existingUser) {
            $contactType = $request->email ? 'email' : 'phone';
            return $this->json(ucfirst($contactType).' user not found', [], Response::HTTP_BAD_REQUEST);
        }

        try {
            // Assign shop role
            // $existingUser->assignRole('shop');
            ...
            //     $logoMedia = MediaRepository::storeByRequest($request->shop_logo, 'shops/logo', 'image');
            // }

            // // Handle shop banner
            // $bannerMedia = null;
            // if ($request->hasFile('shop_banner')) {
            //     $bannerMedia = MediaRepository::storeByRequest($request->shop_banner, 'shops/banner', 'image');
            // }

            // // Create shop if it doesn't already exist
            // $shop = $existingUser->shop ?? ShopRepository::create([
            //     'user_id' => $existingUser->id,
            //     'name' => $existingUser->name,
            //     'status' => true,
            // ]);
            
              $shop = CustomerSellerSetup::ensure($existingUser);

            if ($request->hasFile('shop_logo')) {
                $logoMedia = MediaRepository::storeByRequest($request->shop_logo, 'shops/logo', 'image');
                $shop->update(['logo_id' => $logoMedia->id]);
            }

            if ($request->hasFile('shop_banner')) {
                $bannerMedia = MediaRepository::storeByRequest($request->shop_banner, 'shops/banner', 'image');
                $shop->update(['banner_id' => $bannerMedia->id]);
            }
            ...
            // Generate access token without expiry
            $accessToken = UserRepository::getAccessTokenWithoutExpiry($existingUser);

            return $this->json('Log In Successful', [
                'user' => SellerUserResource::make($existingUser->fresh()),
                'access' => $accessToken,
            ]);
        } catch (\Throwable $th) {
            return $this->json('An error occurred during registration: '.$th->getMessage(), [], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
