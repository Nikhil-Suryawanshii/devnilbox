...
class OrderController extends Controller
{
    ...
    public function store(OrderRequest $request)
    {
        ...
        $cartIds = array_filter($request->cart_ids ?? []);
        $query = $user->customer?->carts()->where('is_buy_now', $isBuyNow);

        if (! empty($cartIds)) {
            $carts = $query->whereIn('id', $cartIds)->get();
        } else {
            $carts = $query->whereIn('shop_id', $request->shop_ids)->get();
        }
        ...
        // push notification
        $deviceKeys = auth()->user()->devices->pluck('key')->toArray();
    ...
    public function reOrder(Request $request)
    {
        ...
        if ($order->order_status->value == OrderStatus::DELIVERED->value) {
            ...
            $message = 'Your re-order has been placed successfully';
