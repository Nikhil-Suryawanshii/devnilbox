                            ...
                            <tr>
                                ...
                                <td>
                                    @if($withdraw->bankAccount)
                                        {{ $withdraw->bankAccount->bank_name }} <br>
                                        <small>{{ $withdraw->bankAccount->iban }}</small>
                                    @else
                                        <span class="text-danger">{{ __('Bank Deleted') }}</span>
                                    @endif
                                </td>
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mt-3">
                            <label class="form-label">
                                {{__('Select Bank')}} <span class="text-danger">*</span>
                            </label>
                            <select name="bank_id" id="bank_id" class="form-control" required>
                                <option value="">{{__('Select Bank')}}</option>
                                @foreach($bankAccounts as $bank)
                                    <option value="{{ $bank->id }}" {{ $bank->is_primary ? 'selected' : '' }}>
                                        {{ $bank->bank_name }} - {{ $bank->iban }}
                                    </option>
                                @endforeach
                            </select>
                            <span class="text-danger" id="bank_id-error"></span>
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <textarea name="message" id="message" placeholder="{{__('Any message')}}" class="form-control"></textarea>
                        </div>
                    </div>
        ...
        $('#withdrawForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                error: function(response) {
                    ...
                    if (errors && errors.bank_id) {
                        $('#bank_id').addClass('is-invalid');
                        $('#bank_id-error').text(errors.bank_id[0]);
                    }
