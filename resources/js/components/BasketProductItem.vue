<template>
    <div class="bg-white flex gap-4 w-full group">
        <div
            v-if="props.product?.cart_id"
            class="shrink-0 flex items-start pt-1 cursor-pointer"
            @click.stop="basketStore.toggleCartItem(props.product.cart_id)"
        >
            <RadioCheckIcon
                :colorClass="basketStore.checkCartIsSelected(props.product.cart_id) ? 'text-primary' : 'text-slate-300'"
            />
        </div>
        <!-- product image -->
