...
const processOrderConfirm = () => {
    ...
    if (basketStore.checkoutProducts.length > 0) {
        ...
        basketStore.syncSelectedShopIds();
        axios.post('/place-order', {
            cart_ids: basketStore.selectedCartIds.map((id) => Number(id)),
            shop_ids: basketStore.getSelectedShopIds(),
...
const fetchCouponApply = () => {
    axios.post("/cart/checkout",
        { ...basketStore.getCheckoutPayload(), coupon_code: coupon.value },
        {
            headers: {
                Authorization: authStore.token,
            },
