<template>
    <div>
        ...
        <TransitionRoot as="template" :show="AuthStore.loginModal">
            <Dialog as="div" class="relative z-[200]" @close="AuthStore.hideLoginModal()">
                <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0"
                    enter-to="opacity-100" leave="ease-in duration-200" leave-from="opacity-100" leave-to="opacity-0">
                    <div class="fixed inset-0 z-[200] bg-gray-500/50 transition-opacity" />
                </TransitionChild>
                                ...
                                <div class="bg-white p-5 sm:p-8 relative" :class="master.langDirection==='rtl' ? 'text-right' : 'text-left'">
                                    ...
                                    <form @submit.prevent="loginFormSubmit()">
                                        ...
                                        <button v-if="!isLoading" type="submit"
                                            class="mt-5 w-full rounded-[10px] bg-orange-500 px-6 py-3 text-base font-medium text-white hover:bg-orange-600">
                                            {{ $t('Log in') }}
                                        </button>
                                        <button v-else type="button"
                                            class="mt-5 flex w-full items-center justify-center gap-1 rounded-[10px] bg-orange-200 px-6 py-3 text-base font-medium text-orange-700" disabled>
                                            {{ $t('Processing') }}
                                            <LoadingSpin />
                                        </button>
                                    </form>
...
onMounted(async () => {
    ...
    try {
        ...
        // Social SDK optional on landing page
    }
});
...
const loginFormSubmit = () => {
    ...
    axios.post('/login', loginFormData.value).then((response) => {
        ...
        const message = error?.response?.data?.message
            || (error?.response?.status === 500 ? 'Server error. Please try again.' : 'Login failed. Check your credentials.')
        toast.error(message, {
