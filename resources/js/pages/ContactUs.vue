                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-2 order-2 md:order-1">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="form-label mb-2">
                                        {{ $t('Email Address') }} <small class="text-red-500">*</small>
                                    </label>
                                    <input type="email" :placeholder="$t('Enter email address')" class="form-input"
                                        v-model="formData.email"
                                        :class="errors && errors?.email ? 'border-red-500' : 'border-slate-200'" />
                                    <span v-if="errors && errors?.email" class="text-red-500 text-sm">{{ errors?.email[0]
                                        }}</span>
                                </div>
                            ...
                            <div class="mt-4">
                                <button type="submit"
                                    class="bg-slate-900 hover:bg-slate-800 text-white px-6 py-4 rounded-lg w-[148px] shadow-lg transition-all active:scale-95">{{ $t('Send') }}</button>
                            </div>
...
const contactFormSubmit = () => {
    ...
    if (formData.value.name && formData.value.email && formData.value.phone && formData.value.subject && formData.value.message) {
